...
class Admin extends MX_Controller
{
    ...
    public function updateoptionalsettings()
    {
        ...
        echo $this->admin_model->updateOptionalSettings($adminlvl, $modlvl, $recaptcha_sitekey, $recaptcha_secret, $register, $smtphost, $smtpport, $smtpcrypto, $smtpuser, $smtppass, $sender, $sendername);
    }
    ...
    public function checkSoap()
    {
        foreach ($this->wowrealm->getRealms()->result() as $charsMultiRealm) {
            echo $this->wowrealm->commandSoap(
                    '.server info',
                    $charsMultiRealm->console_username,
                    $charsMultiRealm->console_password,
                    $charsMultiRealm->console_hostname,
                    $charsMultiRealm->console_port,
                    $charsMultiRealm->emulator
                ) . '<br>';
        }
    }
