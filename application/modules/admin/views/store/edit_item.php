                  ...
                  <div class="uk-inline uk-width-1-2@s">
                    ...
                    <div class="uk-form-controls">
                      <select class="uk-select" id="item_category">
                        ...
                        <option value="<?= $category->id ?>" <?php if ($this->admin_model->getItemSpecifyCategory($idlink) == $category->id) {
                            echo 'selected';
                        } ?>><?= $category->name ?> - <?= $this->wowrealm->getRealmName($category->realmid); ?></option>
                        <?php endforeach; ?>
                      </select>
                    </div>
                  </div>
                  <div class="uk-inline uk-width-1-2@s">
                    ...
                    <div class="uk-form-controls">
                      <select class="uk-select" id="item_type">
                        ...
                        <option value="1" <?php if ($this->admin_model->getItemSpecifyType($idlink) == 1) {
                            echo 'selected';
                        } ?>><?= $this->lang->line('placeholder_item'); ?></option>
                        <option value="2" <?php if ($this->admin_model->getItemSpecifyType($idlink) == 2) {
                            echo 'selected';
                        } ?>><?= $this->lang->line('table_header_money'); ?></option>
                        <option value="3" <?php if ($this->admin_model->getItemSpecifyType($idlink) == 3) {
                            echo 'selected';
                        } ?>><?= $this->lang->line('table_header_level'); ?></option>
                        <option value="4" <?php if ($this->admin_model->getItemSpecifyType($idlink) == 4) {
                            echo 'selected';
                        } ?>><?= $this->lang->line('option_rename'); ?></option>                        
                        <option value="8" <?php if ($this->admin_model->getItemSpecifyType($idlink) == 8) {
                            echo 'selected';
} ?>>Command</option>
                      </select>
                    </div>
                  ...
                  <div class="uk-inline uk-width-1-2@s">
                    ...
                    <div class="uk-form-controls">
                      <select class="uk-select" id="item_price_type">
                        ...
                        <option value="1" <?php if ($this->admin_model->getItemSpecifyPriceType($idlink) == 1) {
                            echo 'selected';
                        } ?>><?= $this->lang->line('option_dp'); ?></option>
                        <option value="2" <?php if ($this->admin_model->getItemSpecifyPriceType($idlink) == 2) {
                            echo 'selected';
                        } ?>><?= $this->lang->line('option_vp'); ?></option>
                        <option value="3" <?php if ($this->admin_model->getItemSpecifyPriceType($idlink) == 3) {
                            echo 'selected';
                        } ?>><?= $this->lang->line('option_dp_vp'); ?></option>
                      </select>
                    </div>
                  </div>
                </div>
...
<div class="uk-margin-small">
    <label class="uk-form-label" id="command_label">
    <?= $this->lang->line('placeholder_command'); ?>
</label>
...
.send money {character} &quot;VMaNGOS Shop&quot; &quot;Enjoy your gold!&quot; 100000"><?= htmlspecialchars($this->admin_model->getItemSpecifyCommand($idlink), ENT_QUOTES, 'UTF-8'); ?></textarea>
...
<div class="uk-margin-small">
    <button class="uk-button uk-button-primary uk-width-1-1" id="button_upitem" type="submit">
        <i class="fas fa-sync-alt"></i> <?= $this->lang->line('button_save'); ?>
    </button>
</div>
<?= form_close(); ?>
          </div>
...
if(type == 8 && command == ''){
    ...
    return false;
        ...
        $.ajax({
          ...
          method:"POST",
          data:{id, name, description, category, type, price_type, dp_price, vp_price, icon, command, [globalThis.csrfName]: globalThis.csrfHash},
          dataType:"text",
          beforeSend: function(){
            $.amaran({
              'theme': 'awesome info',
